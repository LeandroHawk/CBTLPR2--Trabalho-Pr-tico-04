...
public class TrabalhoPratico04 extends JFrame {
    ...
    private int posicaoHistorico = -1; // Posição atual no histórico
    ...
    private void pesquisarFuncionario() {
        ...
        try {
            ...
            if (resultSet.next()) {
                // Adicionar nome ao histórico se for uma nova pesquisa
                if (historicoPesquisas.isEmpty() || !historicoPesquisas.get(historicoPesquisas.size() - 1).equals(nome)) {
                    historicoPesquisas.add(nome);
                    posicaoHistorico = historicoPesquisas.size() - 1; // Atualizar posição atual
                }
                exibirDados();
            } else {
